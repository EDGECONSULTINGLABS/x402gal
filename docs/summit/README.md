...
- Base map is OpenFreeMap's dark vector style (OpenMapTiles, no key, no quota), chosen so its
  ...
  automatic fall-back to OpenStreetMap raster tiles when the style or its tiles error. The stock style
  paints land `rgb(12,12,12)` and water `rgb(27,27,29)` — a black screen — so `MatchMap.tsx` repaints
  it on load (`GROUND`: navy land, blue water, visible roads and borders, legible labels) and adds
  terrain relief from Mapzen/AWS Open Data elevation tiles (also keyless), with its own legend row. The base map
