...
import { DROPLETS_PER_HYDRO } from "./constants";
...
export function quoteBuyHydro(usdcMicros: number): SwapQuote {
  ...
  const outDroplets = amm.reserveHydro - newReserveHydro;
...
export function swapUsdcForHydro(usdcMicros: number): SwapQuote {
  ...
  amm.reserveHydro -= q.outDroplets;
...
// Cost in micro-USDC to acquire `droplets` HYDRO at the current spot price.
// Used to denominate the x402 price the agent pays.
export function dropletsToUsdcMicros(droplets: number): number {
  return Math.max(1, Math.ceil(droplets * priceUSDC()));
}

export function marketCapUSDC(): number {
  const amm = ledger().amm;
  // circulating ≈ initial 10M HYDRO minus retired
  const circulating = 10_000_000 * DROPLETS_PER_HYDRO - amm.totalRetiredDroplets;
