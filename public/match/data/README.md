...
- `esg.geojson` — every facility row in the ESG workbook (414), one point each, one colour. Properties: `id`, `company`, `facility`, `sector`, `goal` (Sustainability/ESG Goal, unedited), `state`, `st`, `city`, `source`, `placement`. The workbook's two fit columns (J, K) and Notes (M) are never read or shipped — Joe confirmed on the 2026-09-08 engineering review they were internal use only, and `npm run build` fails if any of them reaches the output (`scripts/check-fit-absent.ts`). Street Address is used for geocoding only.
...
- `esg-summary.json` — the Summary by State and Summary by Company tabs with per-row `placed` counts, `byPlacement`, and the `approximate` / `unplaced` lists to take back to the workbook owner.
...
- Polygons: `npm run match:clip` (interim, USGS services) or `npm run match:clip -- --source=source-data` once the full delivery (HUC12/10/8 `.shp` + `.dbf` + `.shx` + `.prj`, aquifers) is in `source-data/`. One metro: `npx tsx scripts/clip-summit-geodata.ts --metro=utah` (call `tsx` directly — `npm run … -- --metro=` does not pass the flag through on Windows and re-clips everything).
- Utah's bbox is derived, not drawn: the workbook's own "Salt Lake City" and "Ogden" market rows plus the Stratos site, buffered 30 km (derivation in `lib/match/metros.ts` and `utah/manifest.json → bboxDerivation`). Re-derive before re-clipping if those rows change.
- Fit gate: `npm run match:check-fit` after a `next build` (also runs inside `npm run build`).
