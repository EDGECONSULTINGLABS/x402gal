...
import { meraxisFetch, memoryWallet } from "@/lib/agentSdk";
...
async function main() {
  ...
  const res = await meraxisFetch(`${BASE}/api/ai/chat`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ prompt: "What's the water cost of this query?" }),
    wallet,
    maxDrops: 2_000_000, // refuse to pay more than 2 HYDRO per query
  });
